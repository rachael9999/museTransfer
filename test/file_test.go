...
func TestGenerateChunkFile(t *testing.T) {
	fileInfo, err := os.Stat("test/1.png")
	if err != nil {
		t.Fatal(err)
	}
	// 分片的个数
	chunkNum := math.Ceil(float64(fileInfo.Size()) / float64(chunkSize))
	myFile, err := os.OpenFile("test/1.png", os.O_RDONLY, 0666)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMergeChunkFile(t *testing.T) {
	myFile, err := os.OpenFile("test/1.png", os.O_CREATE|os.O_WRONLY|os.O_APPEND, os.ModePerm)
	if err != nil {
		t.Fatal(err)
	}
	fileInfo, err := os.Stat("test/1.png")
